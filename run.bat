...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f sample-share
	call %MVN_EXEC% clean package -pl sample-share,sample-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill sample-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f sample-acs
	call %MVN_EXEC% clean package -pl sample-integration-tests,sample-platform,sample-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl sample-platform,sample-integration-tests,sample-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl sample-platform,sample-integration-tests
EXIT /B 0
:purge
